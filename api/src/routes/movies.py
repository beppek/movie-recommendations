...
def movies_euclidean_recommendations(_id):
    ...
    data = get_recommendations(movie_based_ratings, mov[_id]['title'], euclidean_distance)
...
def movies_pearson_recommendations(_id):
    ...
    mov = read_movies()
    movie_based_ratings = transform_prefs(ratings)
    data = get_recommendations(movie_based_ratings, mov[_id]['title'], pearson_correlation)
    return jsonify({'recommendations': data})